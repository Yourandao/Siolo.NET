...
namespace Siolo.NET.Components.Neo4j
{
	public class Neo4J
	{
		...
		{
			var query = $@"match (a:Host {{src_ip : '{host}'}}) detach delete a";
		...
		{
			var query = $@"match (a:Host {{src_ip : '{ host.Ip }'}}) detach delete a";
		...
		{
			var query = $@"create (a:Host {{ src_ip : '{ host.Ip }', subnet : '{ host.IsSubnet }'}})";
		...
		public async Task<string> PushHostRaw(string ip)
		{
			...
			var query = $@"create (a:Host {{ src_ip : '{ ip }', subnet : '{ isSubnet }'}})";
		...
		/*
		 * MATCH path=(a:Host{src_ip:'192.168.6.2/32'})-[:KNOWS*1..6]->(t:Host{src_ip:'192.168.5.3/32'}) 
		   unwind relationships(path) as rel
		   return collect({from: rel.from, to: rel.to})

		   match (a:Host{src_ip:'192.168.6.2/32'})-[r:KNOWS*5..6]->(t:Host{src_ip:'192.168.5.3/32'})
		   unwind r as rel
		...
		public async Task CreateRelation(Neo4jHostObject host1, Neo4jHostObject host2)
		{
			...
			{
				var query =
					@$"match (a:Host), (b:Host) where a.src_ip contains '{host1.Ip}' and b.src_ip contains '{host2.Ip}' create (a)-[:KNOWS {{from: a.src_ip, to: b.src_ip}}]->(b), (b)-[:KNOWS{{from: b.src_ip, to: a.src_ip}}]->(a)";
		...
		{
			var query =
				$"match path = (a:Host {{src_ip : '{first}'}})-[:KNOWS*1..6]->(t:Host {{src_ip : '{second}'}}) unwind relationships(path) as rel return collect({{from : rel.from, to : rel.to }}) as PathData";
		...
		{
			var query = "match (n:Host) return n.src_ip as src_ip";
			var result = await ExecuteWithMultipleResult<string>(query, "src_ip");
